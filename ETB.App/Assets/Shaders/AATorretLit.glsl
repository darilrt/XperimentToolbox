...
vec3 direccionalLight = vec3(1, -1, 0);
vec3 normal;
...
vec3 GetNormalFromMap(sampler2D normalMap) {
    vec3 tangentNormal = texture(normalMap, uvMainTex).xyz * 2.0 - 1.0;
...
vec3 Frag(vec2 uvMap) {
	...
	normal = GetNormalFromMap(_NormalMap);
	
	vec3 maskColor = texture(_MaskMap, uvMap).rgb;
	
	metallic = maskColor.g;
	roughness = maskColor.r;
	ao = maskColor.b;
...
void main() {
	gl_FragColor = vec4(Frag(vec2(uvMainTex.x, 1 - uvMainTex.y)), 1);
}
#endif